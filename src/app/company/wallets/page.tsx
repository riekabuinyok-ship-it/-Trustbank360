    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Branch wallet balances</p>
        </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {branchWallets.map((w: any) => (
                <div key={w.id} className="p-4 rounded-lg border bg-surface-50 dark:bg-surface-800/50">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Opening: {formatCurrency(w.openingBalance, w.currency)}</p>
                </div>
              ))}
            </div>
          </CardContent>
